...
@Component({
  selector: 'app-note-discipline-edit-admin',
  templateUrl: './note-discipline-edit-admin.component.html'
})
export class NoteDisciplineEditAdminComponent extends AbstractEditController<NoteDisciplineDto, NoteDisciplineCriteria, NoteDisciplineService>   implements OnInit {
   ...
   public async openCreateDiscipline(discipline: string) {
        const isPermistted = await this.roleService.isPermitted('Discipline', 'edit');
        if(isPermistted) {
             this.discipline = new DisciplineDto();
